...
export const DEFAULT_PRESETS: AccentPreset[] = [
  { key: 'navy',    label: '네이비',    color: '#1a2d4a' },
  { key: 'indigo',  label: '인디고',    color: '#4F46E5' },
  { key: 'rose',    label: '로즈',      color: '#E11D48' },
  { key: 'emerald', label: '에메랄드',  color: '#059669' },
  { key: 'amber',   label: '앰버',      color: '#B45309' },
  { key: 'violet',  label: '바이올렛',  color: '#7C3AED' },
...
export function ThemeProvider({ children }: { children: React.ReactNode }) {
  ...
  const [accentKey, setAccentKeyState]   = useState('navy');
  const [customPresets, setCustomPresets] = useState<AccentPreset[]>([]);
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      if (presetsVal) {
        try { setCustomPresets(JSON.parse(presetsVal)); } catch {}
      }
      setHydrated(true);
    });
  ...
  const colors = useMemo(() => {
    ...
    const accentHex = isCustomHex(accentKey)
      ? accentKey
      : (allPresets.find(p => p.key === accentKey)?.color ?? DEFAULT_PRESETS[0].color);
