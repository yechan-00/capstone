...
One line JSON only:`;
  ...
  if (parsedStore) memoLines.push(`📍 ${parsedStore}`);
  parsedItems.forEach(i => memoLines.push(`· ${i.full} ${i.price.toLocaleString()}원`));
  parsedDiscounts.forEach(d => memoLines.push(`· ${d.name} -${d.amount.toLocaleString()}원`));
